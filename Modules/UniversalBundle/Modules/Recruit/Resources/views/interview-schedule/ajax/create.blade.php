                            ...
                            <select class="form-control select-picker job-app" name="jobStage" id="jobStage"
                                ...
                                @if ($jobId)
                                    @foreach ($selectedStages as $stage)
                                        <option value="{{ $stage->stage->id }}">{{ __('recruit::modules.name.' .str_replace(' ', ' ',  $stage->stage->name)) }}</option>
                                    @endforeach
                                @else
                                    @foreach ($stages as $stage)
                                        <option value="{{ $stage->id }}"> {{  ($stage->name == 'HR round' && $stage->name == 'Technical round' && $stage->name == 'Manager round') ? __('recruit::modules.name.' .str_replace(' ',' ',  $stage->name)) : $stage->name }}</option>
                                    @endforeach
                                @endif
                            </select>
                        </x-forms.input-group>
