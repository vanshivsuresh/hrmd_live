        ...
        <!-- Table Row Start -->
            <tr class="main-table-items text-black">
                ...
                <td width="40%" class="border-bottom-0">
                    {{ $invoice->package->name  }} @if($invoice->package->default != 'trial') - @lang('superadmin.'.$company->package_type) @endif
                </td>
                <td align="right"
                    class="border-bottom-0">{{ $invoice->pay_date?->format($global->date_format) }} - {{ $invoice->next_pay_date?->format($global->date_format) }}</td>
