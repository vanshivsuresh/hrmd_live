...
if (!function_exists('hrm_plugins')) {
    ...
    function hrm_plugins()
    {
        ...
        if (!cache()->has('hrm_plugins')) {
            $plugins = \Nwidart\Modules\Facades\Module::allEnabled();
            cache(['hrm_plugins' => array_keys($plugins)]);
        }

        return cache('hrm_plugins');
    }
...
if (!function_exists('isHrmForSale')) {

    function isHrmForSale()
    {
        return strtolower(config('app.app_name')) === 'hrms-saas';
    }

}

if (!function_exists('isHrms')) {

    function isHrms()
    {
        return strtolower(config('app.app_name')) === 'hrms';
    }
...
if (!function_exists('getDomain')) {
    ...
    {
        if (!$host) {
            $host = $_SERVER['SERVER_NAME'] ?? 'hrmdaddy.in';
        }
